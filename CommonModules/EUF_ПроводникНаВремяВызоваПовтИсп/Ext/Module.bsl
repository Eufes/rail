...
//
// Параметры:
//  ОбъектИнициатор		 - ПланОбменаСсылка.EUF_ПроводникВнешниеСистемы
//						 - СправочникСсылка.EUF_ПроводникПодключенияКВнешнимСистемам
...
Функция ПолучитьПараметрыПодключения(ОбъектИнициатор, ИнтерфейсТранспорта = Неопределено) Экспорт
	...
	Если ТипЗнч(ОбъектИнициатор) = Тип("ПланОбменаСсылка.EUF_ПроводникВнешниеСистемы") Тогда
		ТекстЗапроса = 
		"ВЫБРАТЬ
		|	ВнешниеСистемы.ПараметрыПодключения КАК ПараметрыПодключенияСсылка
		|ПОМЕСТИТЬ ВТ_ПараметрыПодключения
		|ИЗ
		|	ПланОбмена.EUF_ПроводникВнешниеСистемы КАК ВнешниеСистемы
		|ГДЕ
		|	ВнешниеСистемы.Ссылка = &Ссылка";
		
	ИначеЕсли ТипЗнч(ОбъектИнициатор) = Тип("СправочникСсылка.EUF_ПроводникПодключенияКВнешнимСистемам") Тогда		
		...
		|ПОМЕСТИТЬ ВТ_ПараметрыПодключения
		|ИЗ
		|	Справочник.EUF_ПроводникПодключенияКВнешнимСистемам КАК ПараметрыПодключения
		|ГДЕ
	...
	Пароли = EUF_ПроводникШлюзБСПСервер.ПрочитатьДанныеИзБезопасногоХранилища(Выборка.ПараметрыПодключенияСсылка,
																				ПаролиДляПолучения);
...
//
// Параметры:
//  Процесс		 	- СправочникСсылка.EUF_ПроводникИнтеграционныеПроцессы	 - Интеграционный процесс
...
Функция ПолучитьАдресатовИнтеграционногоПроцесса(Процесс, Назначение = "ДляУведомленийОбОшибках", ГруппаАдресатов = "") Экспорт
	...
	|ИЗ
	|	Справочник.EUF_ПроводникИнтеграционныеПроцессы.Адресаты КАК ИнтеграционныеПроцессыАдресаты
	|ГДЕ
	...
	Пока ВыборкаПоТипамАдресатов.Следующий() Цикл
		...
		ПочтовыеАдреса = EUF_ПроводникШлюзБСПСервер.КонтактнаяИнформацияОбъектов(Получатели, ТипКИ, ВидКИ);
...
//
// Параметры:
//  НастройкаСсылка			 - СправочникСсылка.EUF_ПроводникНастройкиИсходящихСообщений,
//								СправочникСсылка.EUF_ПроводникНастройкиВходящихСообщений - Ссылка на справочник настроек
...
Функция РеквизитыНастроекСообщений(НастройкаСсылка, СтрРеквизитыОбработчика) Экспорт
	...
	РеквизитыОбработчика = EUF_ПроводникШлюзБСПСервер.ЗначенияРеквизитовОбъекта(НастройкаСсылка, СтрРеквизитыОбработчика);
...
// 
// Возвращаемое значение:
//  СправочникСсылка.EUF_ПроводникТипВходящегоСообщения, Неопределено - Тип входящего собщения если найден
//
...
Функция ТекущийПользователь() Экспорт
	...
	Возврат EUF_ПроводникШлюзБСПСервер.ТекущийПользователь();
