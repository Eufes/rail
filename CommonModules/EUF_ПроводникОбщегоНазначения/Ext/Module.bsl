...
Функция ОсновнаяВерсияСхемИнтеграции() Экспорт
	...
	//Возврат ПолучитьФункциональнуюОпцию("EUF_ПроводникОсновнаяСхемаИнтеграции");
...
Функция ПриНачалеВыполненияРегламентногоЗадания(РеглЗадание, ИДРеглЗадания, ВнешняяСистема = Неопределено) Экспорт
	...
	Если Не EUF_ПроводникОбмен.РазрешитьРаботуАдаптера(ИДРеглЗадания, ИмяРеглЗадания) Тогда
		...
		EUF_ПроводникВызовСервера.ВыполнитьЛогирование("Операции адаптера заблокированы",
	...
	EUF_ПроводникШлюзБСПСервер.ПриНачалеВыполненияРегламентногоЗадания(РеглЗадание);
...
// Получает необходимые параметры из карточки справочника EUF_ПроводникПодключенияКВнешнимСистемам.  
//
// Параметры:
//
//  ИдентификаторПодключения - Строка - Идентификатор карточки справочника EUF_ПроводникПодключенияКВнешнимСистемам.
...
Функция ПолучитьРазрешенияНаИнтернетРесурс(ИдентификаторПодключения, ИнтерфейсПодключения, Описание) Экспорт
	...
	Если ИнтерфейсПодключения = ДляСервисаHTTP Тогда
		Запрос.Текст =
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	EUF_ПроводникПодключенияКВнешнимСистемам.HTTPАдресСервиса КАК АдресСервиса,
		|	EUF_ПроводникПодключенияКВнешнимСистемам.HTTPРесурсНаСервере КАК РесурсНаСервере,
		|	EUF_ПроводникПодключенияКВнешнимСистемам.HTTPПорт КАК Порт,
		|	ВЫБОР
		|		КОГДА EUF_ПроводникПодключенияКВнешнимСистемам.HTTPЗащищенноеСоединение
		|			ТОГДА ""HTTPS""
		|		ИНАЧЕ ""HTTP""
		|	КОНЕЦ КАК Протокол,
		|	ВЫБОР
		|		КОГДА ПОДСТРОКА(EUF_ПроводникПодключенияКВнешнимСистемам.HTTPАдресСервиса, 1, 4) <> ""http""
		|				И ПОДСТРОКА(EUF_ПроводникПодключенияКВнешнимСистемам.HTTPАдресСервиса, 1, 4) <> """"
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК АдресКорректный
		|ИЗ
		|	Справочник.EUF_ПроводникПодключенияКВнешнимСистемам КАК EUF_ПроводникПодключенияКВнешнимСистемам
		|ГДЕ
		|	EUF_ПроводникПодключенияКВнешнимСистемам.ИдентификаторПодключения = &ИдентификаторПодключения";
				
	ИначеЕсли ИнтерфейсПодключения = ДляСервисаWeb Тогда
		Запрос.Текст =
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	EUF_ПроводникПодключенияКВнешнимСистемам.WSСсылкаWSDL КАК WSСсылкаWSDL,
		|	ВЫБОР
		|		КОГДА ПОДСТРОКА(EUF_ПроводникПодключенияКВнешнимСистемам.WSСсылкаWSDL, 1, 5) = ""https""
		|			ТОГДА ""HTTPS""
		|		КОГДА ПОДСТРОКА(EUF_ПроводникПодключенияКВнешнимСистемам.WSСсылкаWSDL, 1, 4) = ""http""
		|			ТОГДА ""HTTP""
		|		ИНАЧЕ ""Не корректная ссылка WSDL в параметрах подключения "" + (ВЫРАЗИТЬ(&ИдентификаторПодключения КАК СТРОКА(100)))
		|	КОНЕЦ КАК Протокол,
		|	EUF_ПроводникПодключенияКВнешнимСистемам.WSСсылкаWSDL КАК АдресСервиса,
		|	ВЫБОР
		|		КОГДА ПОДСТРОКА(EUF_ПроводникПодключенияКВнешнимСистемам.WSСсылкаWSDL, 1, 5) = ""https""
		|			ТОГДА 9
		|		КОГДА ПОДСТРОКА(EUF_ПроводникПодключенияКВнешнимСистемам.WSСсылкаWSDL, 1, 4) = ""http""
	...
	Иначе
		EUF_ПроводникШлюзБСПКлиентСервер.СообщитьПользователю("Не определен интерфейс транспорта: http-сервис или web-сервис");
	...
	Если Выборка.Следующий() Тогда
		...
		Если Не Выборка.АдресКорректный Тогда
			...
			EUF_ПроводникШлюзБСПКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Возврат СтруктураРазрешений;
		КонецЕсли;
