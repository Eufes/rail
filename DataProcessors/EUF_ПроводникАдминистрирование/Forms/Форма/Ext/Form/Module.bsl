...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СоответствиеОбъектовИФормВР.Вставить("ПараметрыПодключений",
											"Справочник.EUF_ПроводникПодключенияКВнешнимСистемам.ФормаСписка");
	СоответствиеОбъектовИФормВР.Вставить("ТипыИсходящихСообщений",
											"Справочник.EUF_ПроводникТипИсходящегоСообщения.ФормаСписка");
	СоответствиеОбъектовИФормВР.Вставить("ТипыВходящихСообщений",
											"Справочник.EUF_ПроводникТипВходящегоСообщения.ФормаСписка");
	СоответствиеОбъектовИФормВР.Вставить("ИнтеграционныеПроцессы",
											"Справочник.EUF_ПроводникИнтеграционныеПроцессы.ФормаСписка");
	СоответствиеОбъектовИФормВР.Вставить("ВнешниеСистемы",
											"ПланОбмена.EUF_ПроводникВнешниеСистемы.ФормаСписка");
	...
	СоответствиеОбъектовИФормВР.Вставить("НастройкиДляТиповСообщений",
											"РегистрСведений.EUF_ПроводникИспользуемыеНастройкиДляСообщений.ФормаСписка");
	СоответствиеОбъектовИФормВР.Вставить("ПодпискиНаСобытия",
											"РегистрСведений.EUF_ПроводникПодпискиОбмена.ФормаСписка");
...
Процедура СоздатьИсходящееСообщение(Команда)
	...
	Если Не ЗначениеЗаполнено(ВыбраннаяВнешняяСистема) Тогда
		EUF_ПроводникШлюзБСПКлиентСервер.СообщитьПользователю("Не выбрана внешняя система",
				, "ВыбраннаяВнешняяСистема");		
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ВыгружаемыйОбъектСсылка) Тогда
		EUF_ПроводникШлюзБСПКлиентСервер.СообщитьПользователю("Не указан выгружаемый объект",
				, "ВыгружаемыйОбъектСсылка");		
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ТипИсходящегоСообщения) Тогда
		EUF_ПроводникШлюзБСПКлиентСервер.СообщитьПользователю("Не указан тип сообщения",
...
Процедура СоздатьИсходящееСообщениеНаСервере()
	...
	ПараметрыСообщения = EUF_ПроводникОбмен.ИнициализироватьПараметрыСозданияИсходящих(ТипИсходящегоСообщения);
	...
	Если ЗначениеЗаполнено(КодФормированияПроизвольныхДанных) Тогда
		...
		Исключение
			...
			EUF_ПроводникШлюзБСПКлиентСервер.СообщитьПользователю(ТекстОшибок, ,
						"КодФормированияПроизвольныхДанных");
		КонецПопытки;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ИсходящееСообщение) Тогда
		EUF_ПроводникШлюзБСПКлиентСервер.СообщитьПользователю("Не удалось создать сообщение. См. лог адаптера");
		Возврат;	
	КонецЕсли;
...
Процедура ЗарегистрироватьВИсходящейОчереди(Команда)
	...
	Если Не ЗначениеЗаполнено(ТекущееИсходящееСообщение) Тогда 
		EUF_ПроводникШлюзБСПКлиентСервер.СообщитьПользователю("Не определено исходящее сообщение",
...
Процедура ЗарегистрироватьВИсходящейОчередиНаСервере()
	...
	ДанныеОбмена = EUF_ПроводникОбмен.ИнициализироватьПараметрыСозданияИсходящих(ТипИсходящегоСообщения);	
	...
	ДанныеОбмена.Вставить("ТекущийПользователь", EUF_ПроводникШлюзБСПСервер.ТекущийПользователь());
	EUF_ПроводникОбмен.ЗарегистрироватьВИсходящуюОчередь(ДанныеОбмена, ТипИсходящегоСообщения);
...
Процедура ОбработатьИсходящееСообщениеНаСервере(ТекущееИсходящееСообщение, ТипИсходящегоСообщения)
	...
	Если ИспользоватьУказанныйТипИНастройки Тогда
		...
		|////////////////////////////////////////////////////////////////////////////////
		|"
		+ EUF_ПроводникОбмен.ТекстЗапросаИсходящихСНастройкамиОбработки();
		...
		Результат = EUF_ПроводникОбмен.ОбработкаИсходящихСообщений(Запрос); 
		
	Иначе
		
		Результат = EUF_ПроводникОбмен.ПодготовитьОдноИсходящееКВыгрузке(ТекущееИсходящееСообщение, Истина);
...
Процедура ВыгрузитьСообщениеВоВнешнююСистемуНаСервере()
	...
	Если ИспользоватьУказанныйТипИНастройки Тогда
		...
		|////////////////////////////////////////////////////////////////////////////////
		|"
		+ EUF_ПроводникОбмен.ТекстЗапросаИсходящихСНастройкамиВыгрузки();
		...
		Результат = EUF_ПроводникОбмен.ВыгрузитьСообщенияВоВнешнююСистему(Запрос, ВыбраннаяВнешняяСистема);
	Иначе
		Результат = EUF_ПроводникОбмен.ОтправитьОдноСообщение(ТекущееИсходящееСообщение, , Истина);
	КонецЕсли;
...
Функция ВыгрузитьДанныеВФайлНаСервере(СтруктураДанных, ТекущееИсходящееСообщение)
	...
	ДанныеИзХранилища = EUF_ПроводникОбмен.ПолучитьДанныеФорматаТранспорта(ТекущееИсходящееСообщение);
...
Процедура СоздатьВходящееСообщениеНаСервере()
	...
	НастройкиВходящегоОбращения = EUF_ПроводникОбмен.ПолучитьНастройкиВходящего(ТипВходящегоСообщения);	
	Если Не ЗначениеЗаполнено(НастройкиВходящегоОбращения.НастройкаСсылка) Тогда
		...
		EUF_ПроводникШлюзБСПКлиентСервер.СообщитьПользователю(ИнфоДляВнешнейСистемы);												
	КонецЕсли;
	...
	Попытка
		ТекущееВходящееСообщение = EUF_ПроводникОбмен.СоздатьВходящееСообщение(НастройкиВходящегоОбращения,
								ПараметрыОбмена);			
	Исключение
...
Процедура ЗарегистрироватьВоВходящейОчереди(Команда)
	...
	Если Не ЗначениеЗаполнено(ТекущееВходящееСообщение) Тогда
		EUF_ПроводникШлюзБСПКлиентСервер.СообщитьПользователю("Не выбрано входящее сообщение",
...
Процедура ЗарегистрироватьВоВходящейОчередиНаСервере()
	...
	EUF_ПроводникОбмен.ЗарегистрироватьВоВходящуюОчередь(ПараметрыОбмена, НастройкиВходящегоОбращения);	
...
Процедура ОбработатьВходящееСообщениеНаСервере()	
	...
	Если ИспользоватьУказанныйТипИНастройкиВходящего Тогда
		...
		|////////////////////////////////////////////////////////////////////////////////
		|"
		+ EUF_ПроводникОбмен.ТекстЗапросаВходящихСНастройкамиОбработки() 
		...
		EUF_ПроводникОбмен.ДобавитьПоляВыборкиНастроекВходящих(ОператорВыбора);
		
		ТекстЗапроса = СхемаЗапроса.ПолучитьТекстЗапроса();	
		Запрос.Текст = ТекстЗапроса; 
		
		EUF_ПроводникОбмен.ОбработкаВходящихСообщений(Запрос);
		
	Иначе
		
		Результат = EUF_ПроводникОбмен.ОбработатьОдноВходящееСообщение(ТекущееВходящееСообщение, Истина);
...
Процедура ОтчетПоИнтеграционнымВзаимодействиямНаСервере()
	...
	ВнешниеСистемы.Добавить(ПланыОбмена.EUF_ПроводникВнешниеСистемы.ПустаяСсылка());
...
Процедура ИспользованиеУказанныхНастроекИсходящих()
	...
	Если ИспользоватьУказанныйТипИНастройки Тогда
		ЦветГруппы = EUF_ПроводникШлюзБСПКлиентСервер.ЦветСтиля("ЦветИзмененногоПоля");		
	Иначе
...
Функция ПолучитьПоследнийКлючЗаписиПротокола()
	...
	|	ПротоколОбмена.ГодИМесяц КАК ГодИМесяц
	|ИЗ
	|	РегистрСведений.EUF_ПроводникПротоколОбмена КАК ПротоколОбмена
	|
	|УПОРЯДОЧИТЬ ПО
	...
	Если Выборка.Следующий() Тогда
		...
		КлючЗаписи.Вставить("ГодИМесяц", Выборка.ГодИМесяц);
		
		МассивКлюча = Новый Массив;
		МассивКлюча.Добавить(КлючЗаписи);		
		КлючЗаписиРегистра = Новый("РегистрСведенийКлючЗаписи.EUF_ПроводникПротоколОбмена", МассивКлюча);
...
Процедура ТипИсходящегоСообщенияПриИзмененииНаСервере()
	...
	|ИЗ
	|	РегистрСведений.EUF_ПроводникИспользуемыеНастройкиДляСообщений.СрезПоследних(, ТипСообщения = &ТипСообщения) КАК НастройкиСрезПоследних
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Процедура ДекорацияСсылкаНаЗаписьОчередиНажатие(Элемент)
	...
	ОткрытьФорму("РегистрСведений.EUF_ПроводникОчередьИсходящих.ФормаСписка", ПараметрыФормы);
...
Процедура ВыбратьИсходящиеСообщения(Команда)
	...
	ОткрытьФорму("Обработка.EUF_ПроводникАдминистрирование.Форма.ВыборИзЗапроса", ПараметрыКонсолиЗапросов, ЭтотОбъект);
...
Процедура ОбщийЛогАаптераВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "ОбщийЛогАаптераСообщение" Тогда		
		...
		Если ТекДанные <> Неопределено Тогда
			Если ТипЗнч(ТекДанные.Сообщение) = Тип("СправочникСсылка.EUF_ПроводникСообщенияИсходящие") Тогда
			...
			ИначеЕсли ТипЗнч(ТекДанные.Сообщение) = Тип("СправочникСсылка.EUF_ПроводникСообщенияВходящие") Тогда
				ПараметрыОткрытия = Новый Структура("Ключ", ТекДанные.Сообщение);
...
Процедура ОбновитьДоступностьАдаптераНаСервере()
	...
	АдаптерДоступен = EUF_ПроводникОбмен.РазрешитьРаботуАдаптера();
	ЗаголовокКнопки = ?(АдаптерДоступен, "Адаптер включен", "Адаптер заблокирован");
	Картинка = ?(АдаптерДоступен, БиблиотекаКартинок.EUF_ПроводникКругЗеленый, БиблиотекаКартинок.EUF_ПроводникКругКрасный);
