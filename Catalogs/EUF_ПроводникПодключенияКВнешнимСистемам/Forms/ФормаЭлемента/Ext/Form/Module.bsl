...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СоответствиеВкладокИФорматаВР.Вставить(Перечисления.EUF_ПроводникИнтерфейсыТранспорта.WebService,
											"НастройкиПодключенийWS");
	СоответствиеВкладокИФорматаВР.Вставить(Перечисления.EUF_ПроводникИнтерфейсыТранспорта.HttpService,
											"НастройкиПодключенийHTTP");
	СоответствиеВкладокИФорматаВР.Вставить(Перечисления.EUF_ПроводникИнтерфейсыТранспорта.COM,
											"НастройкиПодключенияCOM");
	СоответствиеВкладокИФорматаВР.Вставить(Перечисления.EUF_ПроводникИнтерфейсыТранспорта.ExternalSystem,
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Справочники.EUF_ПроводникПодключенияКВнешнимСистемам.ИдентификаторПодключенияУникален(ТекущийОбъект, Отказ);
	Если Отказ Тогда
		Возврат;			
	КонецЕсли;	
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если WSПарольИзменен Тогда
		EUF_ПроводникШлюзБСПСервер.ЗаписатьДанныеВБезопасноеХранилище(ТекущийОбъект.Ссылка, WSПароль, "WSПароль")
	КонецЕсли;
	
	Если WSПарольОпределенияИзменен Тогда
		EUF_ПроводникШлюзБСПСервер.ЗаписатьДанныеВБезопасноеХранилище(ТекущийОбъект.Ссылка, WSПароль, "WSПарольОпределения")
	КонецЕсли;
	
	Если HTTPПарольИзменен Тогда
		EUF_ПроводникШлюзБСПСервер.ЗаписатьДанныеВБезопасноеХранилище(ТекущийОбъект.Ссылка, HTTPПароль, "HTTPПароль")
	КонецЕсли;
	
	Если COMПарольИзменен Тогда
		EUF_ПроводникШлюзБСПСервер.ЗаписатьДанныеВБезопасноеХранилище(ТекущийОбъект.Ссылка, COMПароль, "COMПароль")
	КонецЕсли;
	
	Если ExtSysПарольИзменен Тогда
		EUF_ПроводникШлюзБСПСервер.ЗаписатьДанныеВБезопасноеХранилище(ТекущийОбъект.Ссылка, ExtSysПароль, "ExtSysПароль")
	КонецЕсли;	
...
Процедура ПрочитатьПаролиИзХранилища(ПаролиФормы, ТекущийОбъект)
	...
	Пароли = EUF_ПроводникШлюзБСПСервер.ПрочитатьДанныеИзБезопасногоХранилища(ТекущийОбъект, ПаролиФормы);
...
Процедура ПолеПароляНачалоВыбора(Элемент, Реквизит, СтандартнаяОбработка) Экспорт
	...
	Если Элемент.РежимПароля Тогда
		Элемент.КартинкаКнопкиВыбора = БиблиотекаКартинок.EUF_ПроводникВводимыеСимволыСкрыты;
	Иначе
		Элемент.КартинкаКнопкиВыбора = БиблиотекаКартинок.EUF_ПроводникВводимыеСимволыВидны;
	КонецЕсли;
...
Процедура ПроверитьПодключениеWS(Результат, ДополнительныеПараметры) Экспорт
	...
	ОчиститьСообщения();		
	ИнтерфейсТранспорта = ПредопределенноеЗначение("Перечисление.EUF_ПроводникИнтерфейсыТранспорта.WebService");
...
Процедура ПроверитьПодключениеHTTP(Команда)
	...
	ОчиститьСообщения();		
	ИнтерфейсТранспорта = ПредопределенноеЗначение("Перечисление.EUF_ПроводникИнтерфейсыТранспорта.HttpService");
	ПроверитьПодключениеНаСервере(ИнтерфейсТранспорта);
...
Процедура ПроверитьПодключениеCOM(Команда)
	...
	ОчиститьСообщения();
	ИнтерфейсТранспорта = ПредопределенноеЗначение("Перечисление.EUF_ПроводникИнтерфейсыТранспорта.COM");
	ПроверитьПодключениеНаСервере(ИнтерфейсТранспорта);
...
Процедура ПроверитьПодключениеExtSys(Команда)
	ОчиститьСообщения();
	ИнтерфейсТранспорта = ПредопределенноеЗначение("Перечисление.EUF_ПроводникИнтерфейсыТранспорта.ExternalSystem");
	ПроверитьПодключениеНаСервере(ИнтерфейсТранспорта);
КонецПроцедуры
...
Процедура ПроверитьПодключениеНаСервере(ИнтерфейсТранспорта)
	...
	Если ИнтерфейсТранспорта = Перечисления.EUF_ПроводникИнтерфейсыТранспорта.WebService Тогда
	...
	ИначеЕсли ИнтерфейсТранспорта = Перечисления.EUF_ПроводникИнтерфейсыТранспорта.HttpService Тогда
	...
	ИначеЕсли ИнтерфейсТранспорта = Перечисления.EUF_ПроводникИнтерфейсыТранспорта.COM Тогда
	...
	ИначеЕсли ИнтерфейсТранспорта = Перечисления.EUF_ПроводникИнтерфейсыТранспорта.ExternalSystem Тогда
	...
	Иначе					
		...
		EUF_ПроводникШлюзБСПКлиентСервер.СообщитьПользователю(ТекстИсключения);
        Возврат;
	КонецЕсли;
	
	Если Не ПарольИзменен И Не Параметры.Ключ.Пустая() Тогда
		УстановитьПривилегированныйРежим(Истина);
		Пароли = EUF_ПроводникШлюзБСПСервер.ПрочитатьДанныеИзБезопасногоХранилища(Объект.Ссылка, ПаролиДляПолучения);
		УстановитьПривилегированныйРежим(Ложь);
	...
	Попытка 			
		Коннектор = EUF_ПроводникОбмен.КоннекторКВнешейСистеме(СтруктураПараметров);
		Если Коннектор <> Неопределено Тогда
			Если СтруктураПараметров.Свойство("КодОтвета") Тогда
				EUF_ПроводникШлюзБСПКлиентСервер.СообщитьПользователю("Есть ограничения (см. код ответа)");
			Иначе	
				EUF_ПроводникШлюзБСПКлиентСервер.СообщитьПользователю("Успешно");
			КонецЕсли; 		
		КонецЕсли;
	Исключение
