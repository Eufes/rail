...
Процедура ОбработатьСообщениеНаСервере(ВходящееСообщение)
	...
	MultiPort_Обмен.ОбработатьОдноВходящееСообщение(ВходящееСообщение);
...
Процедура УстановитьОтборыДинамическихСписков()
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ЕСТЬNULL(СообщенияВходящие.ТипСообщения.ИнтеграционныйПроцесс, ЗНАЧЕНИЕ(Справочник.MultiPort_ИнтеграционныеПроцессы.ПустаяССылка)) КАК ИнтеграционныйПроцесс
	|ИЗ
