...
Процедура СохранитьСхемуВХранилищеОбъектаНаСервере(АдресФайлаВоВременномХранилище, Расширение, Отказ)
	...
	Попытка
		// Проверка архива 
		МенеджерXSD = Справочники.MultiPort_СхемыXSD;
		...
		КонецЕсли;
	Исключение
		MultiPort_ШлюзБСПКлиентСервер.СообщитьПользователю(ОписаниеОшибки(),,,"ПутьКАрхивуСоСхемамиXSD", Отказ);
		Возврат;
	КонецПопытки;
...
Процедура СохранитьСхемуВХранилищеОбъекта(Отказ)
	...
	Иначе
		...
		Если ПустаяСтрока(ПутьКАрхивуСоСхемамиXSD) Тогда
			MultiPort_ШлюзБСПКлиентСервер.СообщитьПользователю(НСтр("ru = 'Путь к файлу не указан!'"),,,"ПутьКАрхивуСоСхемамиXSD", Отказ);
			Возврат;
		КонецЕсли;
		
		Если НЕ СтрЗаканчиваетсяНа(СокрЛП(ПутьКАрхивуСоСхемамиXSD), ".zip") тогда
			MultiPort_ШлюзБСПКлиентСервер.СообщитьПользователю(НСтр("ru = 'Неверный формат файла!'"),,,"ПутьКАрхивуСоСхемамиXSD", Отказ);
			Возврат;
		КонецЕсли;
		
		Файл = Новый Файл(ПутьКАрхивуСоСхемамиXSD);
		Если Не Файл.Существует() Тогда
			MultiPort_ШлюзБСПКлиентСервер.СообщитьПользователю(НСтр("ru = 'Файл не существует!'"),,,"ПутьКАрхивуСоСхемамиXSD", Отказ);
			Возврат;
		КонецЕсли;
...
Процедура СохранитьСхемуВХранилищеОбъектаЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДопПараметры) Экспорт
	...
	Если Результат Тогда
		...
		Если Не Файл.Существует() Тогда
			...
			MultiPort_ШлюзБСПКлиентСервер.СообщитьПользователю(НСтр("ru = 'Файл не существует!'"),,,ПутьКДанным, Отказ);
			Возврат;
