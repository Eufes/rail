...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	//MultiPort_КонструкторСообщенийXML.ПроверитьЗаполнениеДереваСтруктураПолей(ОбДерево, Отказ);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//	
	//	ФабрикаXDTOВерсии = MultiPort_ПовтИсп.ПолучитьФабрикуXDTOПоВерсииСхемИнтеграции(Объект.ВерсияСхемы);
	//
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ИсточникВыбора.ИмяФормы = "Справочник.MultiPort_КонструкторИсходящихXML.Форма.ФормаРедактированияСтроки" Тогда
		ОбработкаИзмененияСтроки(ВыбранноеЗначение);
	ИначеЕсли ИсточникВыбора.ИмяФормы = "Справочник.MultiPort_КонструкторИсходящихXML.Форма.ФормаВыбора" Тогда
...
Процедура ДанныеПолучатьВыполняемымАлгоритмомПриИзмененииНаСервере()
	...
	Иначе
		Объект.АлгоритмПолученияДанных = MultiPort_ОбщегоНазначения.ПреобразоватьТекстАлгоритмаВТекстЗапроса(Объект.АлгоритмПолученияДанных);
	КонецЕсли;
...
Процедура ЗаполнитьСтруктуруПолейНаСервере()
	...
	СтруктураСхем = MultiPort_ПовтИсп.ПолучитьФабрикуXDTOиНаборСхемПоВерсииСхемИнтеграции(Объект.ВерсияСхемы);		
...
Процедура ЗаполнитьДеревоИзПравила(Команда)
	...
	Иначе
		ОткрытьФорму("Справочник.MultiPort_КонструкторИсходящихXML.Форма.ФормаВыбора", , ЭтаФорма, , , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
...
Функция СформироватьТекстЗапросаДляКонфигуратора(Текст)
	...
	ТекстИсполняемогоКода = Справочники.MultiPort_КонструкторИсходящихXML.ПолучитьМакет("ШаблонИсполняемогоКодаДляПолученияДанныхОбъекта").ПолучитьТекст();
...
Функция ПолучитьПакетыТиповСхемыXSD(ВерсияСхемы, ПространствоИмен)
	...
	ФабрикаXDTOВерсии = MultiPort_ПовтИсп.ПолучитьФабрикуXDTOПоВерсииСхемИнтеграции(ВерсияСхемы);
...
Процедура ОткрытьФормуРедактированияСтроки(ВыбранноеЗначение)
	...
	ОткрытьФорму("Справочник.MultiPort_КонструкторИсходящихXML.Форма.ФормаРедактированияСтроки", ПараметрыФормы, ЭтаФорма, , , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ЗаполнитьСписокВыбораЗначенийПолейДерева()
	...
	Если Объект.ДанныеПолучатьВыполняемымАлгоритмом тогда
		ТекстЗапроса = MultiPort_ОбщегоНазначения.ПреобразоватьТекстАлгоритмаВТекстЗапроса(Объект.АлгоритмПолученияДанных);
	КонецЕсли;
	
	СтруктураПолейИсходящихДанных = MultiPort_ОбщегоНазначения.ПолучитьСтруктуруПолейЗапроса(ТекстЗапроса);
...
Процедура ПроверитьЗапрос(Команда)
	ОписаниеОшибки = "";
	//MultiPort_КонструкторСообщенийXML.ПроверитьТекстЗапросНаФорме(СокрЛП(Объект.АлгоритмПолученияДанных), ОписаниеОшибки);
...
Процедура ПроверитьАлгоритмНаСервере()
	...
	ФабрикаXDTOСообщения = MultiPort_ПовтИсп.ПолучитьФабрикуXDTOПоВерсииСхемИнтеграции(Объект.ВерсияСхемы);
