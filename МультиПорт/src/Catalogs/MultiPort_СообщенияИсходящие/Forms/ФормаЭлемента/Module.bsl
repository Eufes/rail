...
Процедура СвязьСДругимиСообщениямиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ТекДанные <> Неопределено Тогда
		Если ТекДанные.Ссылка = Объект.Ссылка Тогда
			Возврат;			
		ИначеЕсли ТипЗнч(ТекДанные.Ссылка) = Тип("СправочникСсылка.MultiPort_СообщенияИсходящие") Тогда
		...
		ИначеЕсли ТипЗнч(ТекДанные.Ссылка) = Тип("СправочникСсылка.MultiPort_СообщенияВходящие") Тогда
			ПараметрыОткрытия = Новый Структура("Ключ", ТекДанные.Ссылка);
...
Процедура ОбработатьСообщениеНаСервере(ИсходящееСообщение)
	...
	MultiPort_Обмен.ПодготовитьОдноИсходящееКВыгрузке(ИсходящееСообщение);
...
Процедура УстановитьПараметрВЗаписиОчереди(ЭлементОтбора = Неопределено)
	...
	Если ТипЗнч(ЭлементОтбора) = Тип("СправочникСсылка.MultiPort_СообщенияИсходящие") Тогда
...
Процедура ОтобразитьПредставлениеДанных()
	...
	Если Не (РольДоступна("ПолныеПрава") Или РольДоступна("MultiPort_Администратор")) Тогда
	...
	Если Не Параметры.Ключ.Пустая() Тогда
		...
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	MultiPort_ДанныеВФорматеТранспорта.ДанныеОбмена КАК ДанныеОбмена,
		|	MultiPort_ДанныеВФорматеТранспорта.ОписаниеФормата КАК ОписаниеФормата
		|ИЗ
		|	РегистрСведений.MultiPort_ДанныеВФорматеТранспорта КАК MultiPort_ДанныеВФорматеТранспорта
		|ГДЕ
		|	MultiPort_ДанныеВФорматеТранспорта.Сообщение = &Сообщение";
