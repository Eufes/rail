...
//
Функция СоединениеСВнешнейСистемой(ОбъектИнициатор, ИнтерфейсТранспорта = Неопределено) Экспорт
...
//
Функция РазрешенаРаботаПодсистемыОбмена(ИДРеглЗадания = "", ИмяРеглЗадания = "") Экспорт
...
Функция НовыйПараметрыИсходящих()
	...
	ПараметрыИсходящих.Вставить("ОтправлятьСразуПослеОбработки", Ложь); 
	...
	ПараметрыИсходящих.Вставить("МетодHTTP", "");
...
Функция ТекстЗапросаИсходящихСНастройкамиОбработки() Экспорт
	...
	|	ЕСТЬNULL(ВЫРАЗИТЬ(ИспользуемыеНастройкиДляСообщенийСрезПоследних.НастройкаДляСообщения КАК Справочник.MultiPort_НастройкиИсходящихСообщений).ОтправлятьСразуПослеОбработки, ЛОЖЬ) КАК ОтправлятьСразуПослеОбработки,
	...
	|	НастройкиИсходящихСообщений.ОтправлятьСразуПослеОбработки,
	...
	|	ЕСТЬNULL(НастройкиПоТипамСообщений.ОтправлятьСразуПослеОбработки, ЛОЖЬ) КАК ОтправлятьСразуПослеОбработки,
...
Функция ТекстЗапросаИсходящихСНастройкамиВыгрузки() Экспорт
	...
	|	ЕСТЬNULL(ИспользуемыеНастройкиДляСообщенийСрезПоследних.НастройкаДляСообщения, НЕОПРЕДЕЛЕНО) КАК НастройкаСсылка,
	|	ИспользуемыеНастройкиДляСообщенийСрезПоследних.ТипСообщения КАК ТипИсходящегоСообщения,
	|	ИспользуемыеНастройкиДляСообщенийСрезПоследних.НастройкаДляСообщения.ОтправлятьСразуПослеОбработки КАК ОтправлятьСразуПослеОбработки,
	...
	|	&КонкретныйТипСообщения,
	|	MultiPort_НастройкиИсходящихСообщений.ОтправлятьСразуПослеОбработки,
	...
	|	ЕСТЬNULL(НастройкиИсходящихСообщений.ОтправлятьСразуПослеОбработки, ЛОЖЬ) КАК ОтправлятьСразуПослеОбработки,
	...
	|	ЕСТЬNULL(НастройкиИсходящихСообщений.РесурсHTTP, """") КАК РесурсНаСервере,
	|	ЕСТЬNULL(НастройкиИсходящихСообщений.МетодHTTP, """") КАК МетодHTTP
	...
	|	СообщенияВместеСНастройками.ОтправлятьСразуПослеОбработки КАК ОтправлятьСразуПослеОбработки,
	...
	|	СообщенияВместеСНастройками.РесурсНаСервере КАК РесурсНаСервере,
	|	СообщенияВместеСНастройками.МетодHTTP КАК МетодHTTP
	|ИЗ
	|	СообщенияВместеСНастройками КАК СообщенияВместеСНастройками
...
Процедура ЗафиксироватьОбработкуИсходящего(ИсходящееСообщение, НастройкиПоТипуСообщения)
	...
	Если Не НастройкиПоТипуСообщения.ОтправлятьСразуПослеОбработки
...
Функция ОбработкаИсходящихСообщений(Запрос, КонкретныйОбработчик = Неопределено) Экспорт
	...
	НастройкиПоТипуСообщения = НовыйПараметрыИсходящих();
...
Функция ОбработатьИсходящееСообщение(ИсходящееСообщение, НастройкиПоТипуСообщения, Отказ = Ложь) Экспорт
	...
	Если НастройкиПоТипуСообщения.ОтправлятьСразуПослеОбработки Тогда
...
Функция ВыгрузитьСообщенияВоВнешнююСистему(Запрос, ВнешняяСистема) Экспорт
	...
	Для Каждого ВариантПодключения Из ТаблицаВариантовПодключений Цикл
		Попытка
			ВариантПодключения.Коннектор = СоединениеСВнешнейСистемой(ВариантПодключения.ПараметрыПодключенияСсылка,
	...
	ПараметрыВыгрузкиИсходящих = НовыйПараметрыИсходящих();
...
Функция ОтправитьСообщениеВоВнешнююСистему(ИсходящееСообщение, ПараметрыВыгрузкиИсходящих, Коннектор) Экспорт
	...
	Иначе	
		Если ОтветВнешнейСистемы = "200" Тогда
			...
			// │ Ключевая (визуальный маркер) 
			//
			Если Не ПараметрыВыгрузкиИсходящих.ОтправлятьСразуПослеОбработки Тогда
...
Функция ПередатьПоHTTP(ИсходящееСообщение, ПараметрыВыгрузкиИсходящих, HTTPСоединение, ПараметрыТранспорта)
	...
	HTTPОтвет = HTTPСоединение.ВызватьHTTPМетод(ПараметрыВыгрузкиИсходящих.МетодHTTP, HTTPЗапрос);
...
Функция ЗарегистрироватьВходящее(ТипВходящегоСообщения, ПараметрыТранспортаВходящего) Экспорт
	...
	Если НастройкиДляВходящего.ОбрабатыватьСразуПриПолучении Тогда
...
Функция ИнициализироватьНастройкиДляВходящих()
	...
	НастройкиВходящегоСообщения.Вставить("ОбрабатыватьСразуПриПолучении", Ложь);
...
Процедура ДобавитьПоляВыборкиНастроекВходящих(ОператорВыбора) Экспорт
	...
	ОператорВыбора.ВыбираемыеПоля.Добавить("НастройкиПоТипамСообщений.ОбрабатыватьСразуПриПолучении");
...
Функция ТекстЗапросаВходящихСНастройкамиОбработки() Экспорт
	...
	|	НастройкиВходящихСообщений.ОбрабатыватьСразуПриПолучении КАК ОбрабатыватьСразуПриПолучении,
...
Процедура ЗафиксироватьОбработкуВходящего(ВходящееСообщение, ПараметрыВыгрузкиВходящих) Экспорт
	...
	Если Не ПараметрыВыгрузкиВходящих.ОбрабатыватьСразуПриПолучении Тогда
...
Процедура ЗафиксироватьОшибкуВходящего(ТекстОшибки, ВходящееСообщение, Заголовок, НастройкиВходящего) Экспорт
	...
	Если Не НастройкиВходящего.ОбрабатыватьСразуПриПолучении Тогда
