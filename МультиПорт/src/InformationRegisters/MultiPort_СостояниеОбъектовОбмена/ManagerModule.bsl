...
Функция УстановитьСостояниеОбъектаОбмена(СтруктураЗаписи) Экспорт
	...
	Для Каждого ПроверяемыйРеквизит Из ПроверяемыеРеквизиты Цикл
		Если СтруктураЗаписи.Свойство(ПроверяемыйРеквизит)
			...
			ОписаниеСобытия = НСтр("ru = 'Добавление записи регистра сведений ""Состояние объектов обмена (MultiPort)""'");
			...
			ЗаписьЖурналаРегистрации(ОписаниеСобытия, 
			                         ...
			                         Метаданные.РегистрыСведений.MultiPort_СостояниеОбъектовОбмена,
			                         ,
			                         Комментарий);
	...
	МенеджерЗаписи = РегистрыСведений.MultiPort_СостояниеОбъектовОбмена.СоздатьМенеджерЗаписи(); 
